...
export function getQuotes(symbols) {
  return cached('q:' + symbols.join(','), QUOTE_TTL_MS, async () => {
    // Report a provider that was skipped for a missing key just as loudly as one
    // that failed — otherwise a misnamed variable looks like the other provider
    // being at fault.
    const errors = [];
    if (twelveDataKey()) {
      try { return await twelveDataQuotes(symbols); } catch (err) { errors.push('TwelveData: ' + err.message); }
    } else {
      errors.push('TwelveData: no key set (expected FOREX_API_KEY or TWELVEDATA_API_KEY)');
    }
    if (process.env.FINNHUB_API_KEY) {
      try { return await finnhubQuotes(symbols); } catch (err) { errors.push('Finnhub: ' + err.message); }
    } else {
      errors.push('Finnhub: no key set (FINNHUB_API_KEY)');
    }
...
export function getCandles(symbol, interval) {
  return cached(`c:${symbol}:${interval}`, CANDLE_TTL_MS, async () => {
    if (!twelveDataKey()) throw new Error('No TwelveData key set (expected FOREX_API_KEY or TWELVEDATA_API_KEY)');
