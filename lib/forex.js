...
async function yahooQuotes(symbols) {
  // Yahoo's unauthenticated endpoint throttles an IP that fires many requests
  // at once, so a full ~70-symbol watchlist in one Promise.allSettled batch
  // mostly comes back empty. Fetching in small batches with a short pause
  // between them stays under that threshold.
  const BATCH_SIZE = 6;
  ...
  for (let i = 0; i < symbols.length; i += BATCH_SIZE) {
    ...
    const results = await Promise.allSettled(
      batch.map(pair => yahooChart(pair, { interval: '5m', range: '1d' }))
    );
    results.forEach((r, j) => {
      const symbol = batch[j];
      if (r.status !== 'fulfilled') { failures.push(`${symbol} (${r.reason.message})`); return; }
      const meta = r.value.meta;
      const price = meta.regularMarketPrice;
      const prev = meta.chartPreviousClose || meta.previousClose;
      if (typeof price !== 'number') { failures.push(`${symbol} (no price in response)`); return; }
      out[symbol] = {
        close: String(price),
